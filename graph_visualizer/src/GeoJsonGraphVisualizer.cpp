...
void GeoJsonGraphVisualizer::parse(const std::vector<common::Segment> &segments) {
...
void GeoJsonGraphVisualizer::addSegment(rapidjson::Document &doc, const common::Segment &segment) {
    rapidjson::Document::AllocatorType& allocator = doc.GetAllocator();
    ...
    for (const auto &point : segment.get_points()) {
        ...
        coord.PushBack(point.longitude, allocator).PushBack(point.latitude, allocator);
