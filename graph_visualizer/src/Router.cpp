...
Router::dijkstra(const common::Segment &startSegment,
  ...
  // graph_visualizer::GeoJsonGraphVisualizer
  // visualizer("checked_segments.json"); visualizer.parse(segments_checked);
...
RouterWithMemoryPool::RouterWithMemoryPool(
...
RouterWithMemoryPool::findShortestPath(const common::GeoPoint &startPoint,
                                       const common::GeoPoint &endPoint) {
  common::MapMatcherWithPool matcher(m_segments);
  common::Segment startSegment = matcher.match(startPoint);
  common::Segment endSegment = matcher.match(endPoint);
  return dijkstra(startSegment, endSegment);
}

void RouterWithMemoryPool::set_cost_calculator(
    std::unique_ptr<router::CostCalculatorStrategy> &cost_calculator) {
  m_cost_calculator = std::move(cost_calculator);
}

void RouterWithMemoryPool::buildConnectorToSegmentsMap() {
  for (const auto &segment : m_segments) {
    for (const auto &connectorId : segment.get_connectors()) {
      m_connectorToSegmentsMap[connectorId].push_back(segment);
    }
...
std::vector<common::Segment, boost::pool_allocator<common::Segment>>
RouterWithMemoryPool::dijkstra(const common::Segment &startSegment,
                               const common::Segment &endSegment) {
  if (!m_cost_calculator) {
    return {};
  }
  ...
  // graph_visualizer::GeoJsonGraphVisualizerWithMemoryPool visualizer(
  //     "checked_segments.json");
  // visualizer.parse(segments_checked);

  std::vector<common::Segment, boost::pool_allocator<common::Segment>> path;
  common::UniqueId currentId = endSegment.get_id();
  while (currentId != startSegment.get_id()) {
    try {
      path.push_back(getSegmentById(currentId));
...
common::Segment
RouterWithMemoryPool::getSegmentById(const common::UniqueId &id) {
  for (const auto &segment : m_segments) {
    if (segment.get_id() == id) {
      return segment;
    }
